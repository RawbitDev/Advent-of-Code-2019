...
#define minPhaseSetting 0
#define maxPhaseSetting 4

void execOpcode(vector <int>* intcode, vector <int>* input = new vector <int>, vector <int>* output = new vector <int>, bool silentMode = false) {
    unsigned int i=0;
    unsigned int inputCounter = input->size();
    ...
    while( i<intcode->size() ) {
        ...
        switch (opcode)
        {
            ...
            case 3: { // Input
                int tmpInput = 0;
                if(inputCounter > 0) { // If there's still any given input use it
                    tmpInput = input->at( input->size()-inputCounter );
                    --inputCounter;
                } else { // Get the input by the user
                    cout << "> ";
                    cin >> tmpInput;
                }
                *param1 = tmpInput;
...
int runSingleAmplifier(vector <int>* amplifierCode, int phaseSetting, int lastAmplifierOutput = 0) {
    vector <int> currentInput = {phaseSetting, lastAmplifierOutput};
    vector <int> currentOutput;
    execOpcode(amplifierCode, &currentInput, &currentOutput, true);
    return currentOutput.back();
}
...
int runAmplifierSequence (vector <int>* originalAmplifierCode, vector <unsigned int>* amplifierSequence) {
    ...
    for(unsigned int i=0; i<amplifierSequence->size(); ++i) {
        result = runSingleAmplifier(new vector <int> (*originalAmplifierCode), amplifierSequence->at(i), result);
    }
    return result;
}
